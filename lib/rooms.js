...
const DEFAULTS = {
  ...
  maxNoteLength: 10,        // 每格共享備註最長幾個字
  maxNotesPerCell: 24,      // 每格最多保留幾位成員的備註
  maxNameLength: 12,        // 暱稱最長幾個字
  chatMinIntervalMs: 700,   // 兩則訊息之間至少要隔多久
  chatWindowMs: 10000,      // 頻率限制的觀察窗
  chatMaxPerWindow: 8,      // 觀察窗內最多幾則
  noteMinIntervalMs: 400,   // 備註更新間隔，避免刷爆 SSE
  noteWindowMs: 10000,      // 備註頻率限制的觀察窗
  noteMaxPerWindow: 30,     // 觀察窗內最多幾次備註更新
...
function createStore(options) {
  ...
  function createRoom(input) {
    ...
    const room = {
      ...
      noteVersion: 0,
      noteLog: new Map()       // token -> 最近更新備註時間陣列（頻率限制用）
    };
    rooms.set(code, room);
  ...
  /* ---------- 共享格子備註 ----------
   * 每位成員在每格保留一則自己的備註，更新會取代自己的舊內容，清空則移除。
   * 備註不是盤面狀態，不會混進主持人的 state 快照；所有人透過 note 事件看到同一份清單。 */
  function allowedNoteByRate(room, senderToken) {
    ...
    if (list.length && t - list[list.length - 1] < cfg.noteMinIntervalMs) {
      return { ok: false, code: 'toofast', status: 429, message: '備註更新太快了，稍等一下再送。' };
    }
    if (list.length >= cfg.noteMaxPerWindow) {
      return { ok: false, code: 'toomany', status: 429, message: '短時間內更新太多備註了，等幾秒再繼續。' };
    }
    list.push(t);
  ...
  function updateCellNote(code, senderToken, rawIndex, rawText, rawName) {
    ...
    if (!room) return { ok: false, code: 'nosuch', status: 404, message: '找不到這個房間，可能已經關閉了。' };
    if (room.status === 'closed') {
      return { ok: false, code: 'closed', status: 410, message: '這個房間已經關閉了，備註沒有送出。' };
    }
    ...
    if (typeof rawText !== 'string') {
      return { ok: false, code: 'text', status: 400, message: '備註格式不正確。' };
    }
    if (rawText.trim().length > cfg.maxNoteLength) {
      return { ok: false, code: 'toolong', status: 400, message: '每格備註最多 ' + cfg.maxNoteLength + ' 個字。' };
    }
    ...
    const existing = list.find((note) => note.authorId === member.id);
    if (!text) {
      if (!existing) {
        return { ok: true, removed: true, index: index, version: room.noteVersion, notes: list.slice() };
      }
      ...
      if (list.length >= cfg.maxNotesPerCell) {
        return { ok: false, code: 'full', status: 409, message: '這格的共享備註已經滿了，請先等別人清除。' };
      }
      list.push({
        id: 'n' + (++noteSeq),
        authorId: member.id,
    ...
    return { ok: true, removed: !text, index: index, version: room.noteVersion, notes: event.notes };
  }
