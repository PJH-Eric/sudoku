...
function createStore(options) {
  ...
  /* ---------- 同一房間開始下一局 ----------
   * 房號、邀請連結、主持人與觀戰者都保留，只替換題目及這一局的盤面資料。
   * 題目留言屬於上一局，必須清掉；聊天室則保留，讓房間不會因為換題而中斷。 */
  function restartRoom(code, hostToken, input) {
    const room = getRoom(code);
    if (!room) return { ok: false, code: 'nosuch', status: 404, message: '找不到這個房間，可能已經關閉了。' };
    if (room.hostToken !== hostToken) {
      return { ok: false, code: 'forbidden', status: 403, message: '只有開房的人可以開始下一局。' };
    }
    if (room.status === 'closed') {
      return { ok: false, code: 'closed', status: 410, message: '這個房間已經關閉了。' };
    }
    ...
    const t = now();
    room.difficulty = typeof data.difficulty === 'string' ? cleanText(data.difficulty, 12) : room.difficulty;
    room.label = cleanText(data.label, 12) || '';
    room.technique = cleanText(data.technique, 24) || '';
    room.seed = cleanText(data.seed, 12).toUpperCase();
    room.snapshot = snapshot;
    room.version++;
    room.status = snapshot.status === 'won' ? 'done' : 'live';
    room.doneAt = snapshot.status === 'won' ? t : 0;
    room.updatedAt = t;
    room.lastActiveAt = t;
    room.hostSeenAt = t;
    room.cellNotes = Array.from({ length: CELLS }, () => []);
    room.noteLog.clear();

    const state = stateEvent(room);
    emit('state', room.code, state);
    emit('roomlist', room.code, null);
    return { ok: true, version: room.version, state: state };
  }
